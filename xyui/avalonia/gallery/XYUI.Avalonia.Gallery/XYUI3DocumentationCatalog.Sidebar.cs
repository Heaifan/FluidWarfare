...
public static partial class XYUI3DocumentationCatalog
{
    static XYUI1ComponentDocument BuildSidebarDoc(string id, string type) => new(
        ...
        "用于玄域引擎桌面主界面左侧；展开态宽 212~240 DIP，折叠态收拢为 54 DIP 紧凑轨，Footer 始终置底吸附。",
        () => XYUI3GalleryCatalog.CreatePreview(id),
        ["<c:XYSidebar PrimaryItems=\"{Binding Primary}\" ContextItems=\"{Binding Context}\" />"],
        [new("Expanded (展开)", "标准 212 DIP 容器：Header + 主导航 + 上下文树/工具 + 置底设置", "桌面标准模式"), new("Collapsed (折叠)", "紧凑 54 DIP 轨：仅保留一级图标、展开触发器与置底入口", "空间紧凑模式")],
...
""",
        ...
        [
            ...
            new("响应式边界", "DO: 留出与 3.07 NavigationRail 的响应式平滑过渡接口。", "DON'T: 在本轮提前把 3.07 NavigationRail 完整实现绑定进来。", "严格守住本轮 3.01~3.06 交付边界。")
        ],
