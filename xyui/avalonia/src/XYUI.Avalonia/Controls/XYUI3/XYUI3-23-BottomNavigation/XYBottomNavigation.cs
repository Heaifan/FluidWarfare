...
public sealed class XYBottomNavigation : Border
{
    ...
    public double SafeAreaBottom { get => _safeAreaBottom; set { _safeAreaBottom = Math.Max(0, value); Height = 66 + _safeAreaBottom; _grid.Margin = new Thickness(0, 0, 0, _safeAreaBottom); } }
    ...
    void ConfigurePrimary() { if (PrimaryAction is null) return; PrimaryAction.Classes.Add("xyui-bottom-navigation-primary"); PrimaryAction.Width = 54; PrimaryAction.Height = 54; PrimaryAction.IsHitTestVisible = true; PrimaryAction.HorizontalAlignment = HorizontalAlignment.Center; PrimaryAction.VerticalAlignment = VerticalAlignment.Top; if (PrimaryAction.Content is XYIcon icon) icon.Classes.Add("xyui-bottom-navigation-primary-icon"); PrimaryAction.Click += (_, _) => PrimaryActionRequested?.Invoke(this, EventArgs.Empty); _primaryHost = new Grid { Classes = { "xyui-bottom-navigation-primary-host" }, RowDefinitions = new RowDefinitions("*,Auto"), HorizontalAlignment = HorizontalAlignment.Stretch, VerticalAlignment = VerticalAlignment.Stretch, ClipToBounds = false, Margin = new Thickness(0, -14, 0, 0) }; _primaryHost.Children.Add(PrimaryAction); var label = new TextBlock { Text = "新建", Classes = { "xyui-bottom-navigation-primary-label" }, HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Bottom }; Grid.SetRow(label, 1); _primaryHost.Children.Add(label); }
