...
public sealed partial class XYCommandPalette
{
    ...
    void Refresh()
    {
        var query = ParseQuery(SearchBox.Text?.Trim() ?? ""); var scope = query.Scope ?? ScopeFilter; var source = string.IsNullOrEmpty(query.Search) ? (RecentItems.Count == 0 ? Commands : RecentItems) : Commands;
        ...
        for (var i = 0; i < FilteredCommands.Count; i++) { var item = new XYCommandPaletteItem(FilteredCommands[i]) { IsSelected = i == _selected }; var index = i; item.PreviewRequested += (_, _) => Select(index); item.Invoked += (_, _) => { Select(index); Execute(item.Command); }; _results.Children.Add(item); } if (FilteredCommands.Count == 0) _results.Children.Add(_emptyState);
    ...
    void OnPaletteKeyDown(object? sender, KeyEventArgs e) { var wantsControl = OpenShortcut.StartsWith("Ctrl", StringComparison.OrdinalIgnoreCase); var wantsMeta = OpenShortcut.StartsWith("Cmd", StringComparison.OrdinalIgnoreCase) || OpenShortcut.StartsWith("Meta", StringComparison.OrdinalIgnoreCase); var pressed = (wantsControl && (e.KeyModifiers & KeyModifiers.Control) != 0) || (wantsMeta && (e.KeyModifiers & KeyModifiers.Meta) != 0); if (e.Key == Key.K && pressed) { Open(); e.Handled = true; } }
